...
class TravelBudget:
    ...
    def for_role(cls, turn: Turn, role: Unit, config: StrategyConfig):
        ...
        assignment=next((a for a in assign_controllers(turn,weapons) if a.controller.unit_id==role.unit_id),None)
        ...
        return cls(grid,role.pos,distance_field(grid,goals),turn.rounds_until_night,
                   config.recall_safety_buffer+config.recall_traffic_buffer,turn.is_day)
