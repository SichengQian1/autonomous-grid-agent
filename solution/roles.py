...
class GuardHandover:
    ...
    def coordinate(self,turn,weapons,config,want_away=False,blocked_ids=()):
        ...
        if incumbent and incumbent.unit_id!=target.unit_id:
            if target.pos not in stages:
                ...
                if available:intents.append(MoveIntent(target.unit_id,available,130))
                self.phase='replacement_approach'
            else:
                vacate=tuple(p for p in stages if p not in occupied)
                if vacate:intents.append(MoveIntent(incumbent.unit_id,vacate,131))
                self.phase='vacate_common_post'
        else:
            intents.append(MoveIntent(target.unit_id,(post,),131));self.phase='enter_common_post'
