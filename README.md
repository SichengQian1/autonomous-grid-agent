...
The v0.20 candidate implementation is active. It includes
...
The wall engineer stocks surplus-funded Bombs by day after defense supplies. One engineer builds and maintains walls, targets seven rebuilding stones, and
sells carried non-stone ore before a timed wall-supply and return trip. Two level-one rear shutters close the repair corridor; workers open an owned shutter, cross and reseal by day, including the final-night helper. The other worker mines and joins final-night wall defense; the pioneer
...
v0.20 uses one shared pioneer gun post, stable wall support, two-day ore holding,
...
two opening attempts and platform feedback bound execution. v0.20 waits for a complete
model candidate before buying offerings, keeps unfinished gun upgrades active,
targets at least five repair packs from day three, and allows cross-midline
mining with path-based dusk recall for every worker. See [the operating guide](docs/operating-logic.md).
...
For `v0.20`, this produces `submissions/v0/submission-v0.20.tar.gz`. The archive
...
The decoder accepts v0.2–v0.20 formats and decoded JSONL, including `.xz` files.
